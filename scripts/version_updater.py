...
def update_patch_count(version_file, tagged_commit_file, force_commit):
    ...
    # Check the last commit, if the last commit contains version updates, no patch count update here!
    # This avoids endless version bump-up!
    last_commit_log_str = subprocess.check_output(
        ["git", "log", "--no-merges", "--oneline", "-1"]
    ).decode(sys.stdout.encoding)
    logging.debug(f"Git log:\n{last_commit_log_str}")
    if old_ver_str in last_commit_log_str.rstrip():
        logging.info(f"Skip version bump-up as last commit is already it!")
        return num_err
